...
export default function TermsPage(): React.JSX.Element {
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#0b1220_0%,_#111827_38%,_#020617_100%)] px-6 py-10 text-white sm:px-10 lg:px-16">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        <section className="rounded-[32px] border border-white/10 bg-white/[0.06] p-8 shadow-2xl shadow-sky-950/25 backdrop-blur sm:p-10">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/"
                className="inline-flex h-11 items-center justify-center rounded-full border border-white/15 bg-slate-950/45 px-5 text-sm font-semibold text-slate-50 transition hover:bg-slate-900/70"
              >
                返回首页
              </Link>
              <Link
                href="/privacy"
                className="inline-flex h-11 items-center justify-center rounded-full border border-sky-200/30 bg-sky-300 px-5 text-sm font-semibold text-slate-950 shadow-lg shadow-sky-950/20 transition hover:bg-sky-200"
              >
                查看隐私政策
              </Link>
            </div>
        ...
        <section className="space-y-6 rounded-[28px] border border-white/10 bg-slate-950/45 p-6 shadow-xl shadow-black/20 sm:p-8">
          ...
          <article className="space-y-3">
            ...
            <code className="block rounded-2xl bg-black/30 px-4 py-3 text-sm text-sky-100">
              contact@example.com
            </code>
          </article>
        </section>
